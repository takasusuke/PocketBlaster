...
namespace PocketBlaster.Aim
{
    ...
    public class GyroReticleController : MonoBehaviour
    {
        ...
        private void HandleShoot()
        {
            ...
            // 「構える」ボタンを押していない間も撃てる(オーナー要望、2026-09-06:
            // 「構えていない状態でも撃つボタンを押したら、中央に撃つようにしてください」)。
            // 当初は構えている間だけに限定していたが、狙いを付けずに素早く中央へ
            // 撃ちたい場面(近くの敵・とっさの反撃)のために解禁した。狙点は
            // Update()が構えていない間ずっと画面中央へ固定しているため、
            // `_reticleScreenX/Y`(GetAimRay参照)は既に中央を指している——ここで
            // 追加の分岐は不要。
