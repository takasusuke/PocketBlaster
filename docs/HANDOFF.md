...
マズルフラッシュ自体はCameraのTransformに触れない画面全体のUIオーバーレイで
実装できる(画面シェイクだけが「シェイク用オフセットを最後に上乗せする」設計変更を
要する、という制約を持つ)ことに気づき、こちらだけ切り離して実装した。

- `GyroReticleController`/`MultiplayerAimController`の両方に、発射(`HandleShoot`)の
  たびに0.05秒だけ画面全体を薄く(アルファ最大0.25)白く光らせる
  `VisualElement`オーバーレイを追加。命中の有無に関わらず発火する
  (ヒットマーカーは命中時だけ、こちらは発射自体のフィードバック)。
  `pickingMode = PickingMode.Ignore`でクリック/タッチ判定を奪わないようにした。
- マルチプレイヤーは画面共有(2人が同じ画面を見る)方式のため、両プレイヤーの
  オーバーレイが同じ画面に重なりうるが、アルファが低く一瞬(0.05秒)なので
  同時発射でも過度に眩しくはならない設計。
- ランタイムのUI要素追加のみで新規`[SerializeField]`は増えていないため、
  **シーンの再ビルドは不要**。EditMode 83件全て通過を確認。
- **未検証**: 実機での明るさ・持続時間の体感(眩しすぎ/短すぎないか)。
  画面シェイクは今回も見送ったまま(docs/requirements.md参照)。
...
- **ステージ終了後は起動画面へ自動復帰**（オーナー要望「ステージ終了後は起動画面に
  戻れるようにしてください」）: `StageDirector.ShowStageClear`と`GameSession`の
  ゲームオーバー処理、それぞれ独立に数秒待ってから`SceneManager.LoadScene("Title")`
  するコルーチンを追加した。
- **起動画面の感度スライダーに"−"/"＋"ボタンを追加**（オーナー要望「起動画面で感度を
  上下されるボタンをスマホから狙って撃つことで調整できるようにしてください」）:
  スマホの「狙って撃つ」は離散的なクリックしかできずスライダーを連続ドラッグできない
  ため、可動域の1/20刻みで調整できるボタンを添えた(`BuildAdjustableSlider`)。
- **構えていない時の視界回転感度を5倍**（オーナー要望「構えていない状態の感度を5倍に
  してください」）: `GameSettingsState`の`LookSensitivity`既定・上下限を範囲ごと
  5倍にした(既定60→300、範囲20-120→100-600)。
- **構えていない状態でも中央へ発砲できるように**（オーナー要望「構えていない状態でも
  撃つボタンを押したら、中央に撃つようにしてください」）: `GyroReticleController.
  HandleShoot`の「構えていないと撃てない」ガードを削除した。構えていない間はもともと
  レティクルを画面中央へ固定表示していたため、追加の分岐なしでそのまま中央へ飛ぶ。
- **移動速度3倍・移動範囲6倍**（オーナー要望）: `PlayerLocomotion.stepDistance`
  0.7→2.1m、`maxOffsetRadius`30→180mに変更。床(GroundFactory)のグリッドサイズも
  400mへ拡大し、範囲外に出て何も無い空間を歩くことがないようにした。
- **フィールドオブジェクトの拡充・パルクール構造物**（オーナー要望「他にもオブジェクトを
  配置してください」「パルクールなども実装してください」）: 新規`FieldObstacleScatterer`
  (Editor)を追加。`Scatter`は固定シードで低い箱・壁・足場を各ステージへ40個、半径
  40〜170mの外周にばら撒く(乱数だがシード固定なので再ビルドしても同じ配置になる)。
  `BuildParkourStaircase`は高さ1→2→3→4mの足場を隣接させ、実際に歩いて登れる一本道を
  作る。両方をMilestone4_Stage・Stage2_BossRushに追加した。
- **練習モード新設**（オーナー要望「敵は出てこず、ただ移動して、構えて撃つだけの練習を
  するモードを実装してください。このモードには、動かない的をステージ上にいくつか
  配置してください」）: 新規シーン`PracticeRange`(`PracticeRangeSceneBuilder`)。
  ウェーブ・スコア・クリア判定(StageDirector)を一切持たず、動かない的
  (`Target`、respawns:true・approaches:false)を6体配置しただけ——的は倒れても自動で
  復帰するので回数無制限に狙う練習ができる。起動画面に3つ目の開始ボタンとして追加した。
- **確認方法**: EditMode 62件全て通過。Milestone3_ShootTarget・Milestone4_Stage・
  Stage2_BossRushの3シーンを再ビルド(PlayerLocomotionの新しいデフォルト値・
  フィールドオブジェクトを反映させるため)し、PracticeRangeを新規ビルド。シーンYAML上で
  意図した内容(FieldObstacle 40個・Parkour_Step 4個・stepDistance 2.1・
  maxOffsetRadius 180・Target 6体)を確認した。**実機(Play Mode)での動作確認は
  まだ行っていない**——移動速度・移動範囲・視界回転感度・追加オブジェクトの配置密度・
  パルクールの登りやすさ・練習モードの的の距離感、いずれも未検証。
- **作業メモ**: 再ビルド前にこのプロジェクトを開いたままのGUI Editorが残っていて
  バッチ実行をブロックしていたため、`taskkill`で閉じてから実行した(このセッションで
  繰り返し発生している——GUIを再度開く前に必ず閉じ忘れがないか確認すること)。
